...
app.get('/', (req, res) => {
  ...
  res.setHeader('Expires', '0');
  res.setHeader('Surrogate-Control', 'no-store');
  res.type('html').send(EMBEDDED_HTML.replace('<\/body>', "<script>(function(){var _t=setInterval(function(){var ab=document.getElementById('add-deal-btn-sidebar');var rb=document.getElementById('ref-btn');var rw=document.getElementById('rewrite-all-btn');if(!ab||!rb||!rw)return;clearInterval(_t);var wrap=ab.parentElement;if(wrap){wrap.style.gap='6px';wrap.style.padding='0 10px 10px';}ab.textContent='+';ab.style.cssText+=';font-size:17px;height:34px;line-height:1;';rb.innerHTML='&#8635;';rb.style.cssText+=';font-size:17px;height:34px;line-height:1;';rw.innerHTML='&#9889;';rw.style.cssText+=';font-size:17px;height:34px;line-height:1;';var neg=document.getElementById('neg-ladder');if(neg)neg.remove();if(typeof renderList==='function'){var _rl=renderList;window.renderList=function(){_rl.apply(this,arguments);document.querySelectorAll('[data-uid]').forEach(function(row){var uid=row.dataset&&row.dataset.uid;if(!uid)return;var d=window.deals&&window.deals.find(function(x){return x.uid===uid;});if(!d||!d.dateReceived)return;var hrs=Math.floor((Date.now()-new Date(d.dateReceived))/3600000);if(hrs<24){row.querySelectorAll('[class*=\"age\"],[class*=\"chip\"],[class*=\"badge\"]').forEach(function(el){if(el.childElementCount===0&&el.textContent.includes('Today')){el.textContent=el.textContent.replace('Today',hrs+'h');}});}});};},300);})();</script>" + '<\/body>'))
});
