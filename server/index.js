...
// Updated: 2026-06-16T16:51:46.368Z
...
app.get('/api/deals', auth, async (req, res) => {
  try {
    ...
    const targetDeals = deals.filter(d => {
      if (!d.county && !d.city) return true; // include if no county data
      return isTargetCounty(d.county || d.city);
    });
    ...
    if (dbCount > 0) {
      ...
      const targetFromDB = Object.fromEntries(
        Object.entries(fromDB).filter(([k, v]) => {
          const county = v.deal?.county || v.deal?.city || '';
...
async function serverAutoUnderwrite() {
  ...
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      const _countyPrecheck = col('County') >= 0 ? (row[col('County')] || '') : (col('City') >= 0 ? (row[col('City')] || '') : '');
      if (_countyPrecheck && !isTargetCounty(_countyPrecheck)) {
        // console.log(`⏭️ Skipping non-target county: ${_countyPrecheck} — ${addr}`);
        continue;
      }
      {
