...
function isTargetCounty(county, city) {
  ...
  if (city) {
    ...
    return false; // city known but not in target → exclude
  }
...
// ── HUD FAIR MARKET RENTS — Tampa-St.Pete-Clearwater MSA (FY2025/2026) ─────────
// All four CCG counties (Hillsborough, Pinellas, Pasco, Hernando) are in ONE MSA.
// Hillsborough & Pinellas use zip-level SAFMRs — values below are MSA-wide baseline.
// Source: HUD FY2025 Schedule of Metropolitan FMRs (verified)
const HUD_FMR = {
  tampa_msa: { eff:1566, '1':1686, '2':1978, '3':2533, '4':3082, '5':3544 },
  // Zip-level SAFMR examples (Hillsborough / Pinellas vary widely by neighborhood)
  // Lower-income East Tampa / inner-city zips run ~10-20% below MSA
  // Pasco & Hernando run ~15-25% below MSA
  pasco:     { eff:1200, '1':1296, '2':1520, '3':1947, '4':2370, '5':2726 },
  hernando:  { eff:1100, '1':1188, '2':1393, '3':1783, '4':2170, '5':2496 },
};

// Look up HUD FMR for a given county and bedroom count
function getHudFmr(county, beds) {
  const c = (county||'').toLowerCase();
  const table = c.includes('pasco') ? HUD_FMR.pasco
               : c.includes('hernando') ? HUD_FMR.hernando
               : HUD_FMR.tampa_msa; // Hillsborough, Pinellas, default
  const b = Math.min(Math.max(parseInt(beds)||1, 0), 5);
  return table[String(b)] || table['3'];
}

// ── UNIT MIX INFERENCE — given total beds/baths/units, return beds per unit ─────
// e.g. 3/2 duplex → [1, 2] meaning 1BR unit + 2BR unit
function inferUnitMix(beds, baths, units) {
  const b = parseInt(beds) || 0;
  const u = parseInt(units) || 1;
  if (u <= 1) return [b];

  // Common duplex patterns
  if (u === 2) {
    if (b <= 2) return [1, 1];
    if (b === 3) return [1, 2];   // 3/2 duplex → 1BR + 2BR
    if (b === 4) return [2, 2];
    if (b === 5) return [2, 3];
    if (b >= 6)  return [3, 3];
  }
  ...
  if (u === 4) {
    if (b <= 4) return [1, 1, 1, 1];
    if (b === 5) return [1, 1, 1, 2];
    if (b === 6) return [1, 1, 2, 2];
    if (b >= 8)  return [2, 2, 2, 2];
  }
  // Generic: distribute as evenly as possible
  const base = Math.floor(b / u);
  const extra = b % u;
  return Array.from({length: u}, (_, i) => i < extra ? base + 1 : base);
}

// ── CALCULATE TOTAL RENT FROM UNIT MIX + HUD FMR ────────────────────────────────
function calcRentFromUnitMix(beds, baths, units, county) {
  const mix = inferUnitMix(beds, baths, units);
  const rents = mix.map(br => getHudFmr(county, br));
  return {
    mix,           // [1, 2] for 3/2 duplex
    rents,         // [1686, 1978] from HUD FMR
    total: rents.reduce((s, r) => s + r, 0),
    perUnit: rents  // each unit's rent separately
  };
}
...
HUD FAIR MARKET RENTS — Tampa-St.Pete-Clearwater MSA (FY2025, all 4 CCG counties):
Efficiency: $1,566 | 1BR: $1,686 | 2BR: $1,978 | 3BR: $2,533 | 4BR: $3,082 | 5BR: $3,544
NOTE: Hillsborough/Pinellas use zip-level SAFMRs — actual zip rents may be 10-25% lower in lower-income areas.
Pasco/Hernando run ~15-25% BELOW MSA average. Adjust down for those counties.

CRITICAL MULTI-UNIT RENT RULE: For duplexes, triplexes, quads — you MUST calculate rent per unit based on
THAT UNIT'S bedroom count, NOT an average. Apply HUD FMR for each unit separately.
EXAMPLE: 3/2 duplex = 1BR unit ($1,686) + 2BR unit ($1,978) = $3,664 MSA total (adjust for zip/county).
DO NOT return $1,686/unit × 2 = $3,372. DO NOT return a flat per-unit average. Each unit gets its own FMR.
Unit mix for 3/2 duplex: 1BR + 2BR. For 4/2 duplex: 2BR + 2BR. For 4/3 triplex: 1BR + 1BR + 2BR.

RENTAL MARKET DATA (live-fetched):
${rentalMarketData || 'No live rental data — use HUD FMR table above adjusted for county/zip.'}
...
PUT THESE FIELDS FIRST — they are most important:
{
  ...
  "rental": {
    "marketRent": {
      "unitCount": <number — 1 for SFR, 2 for duplex, 3 for triplex, 4 for quad. Detect from propertyType.>,
      "unitMix": <array of bedroom counts per unit — e.g. [1,2] for 3/2 duplex, [2,2] for 4/2 duplex, [1,1,1] for 3/3 triplex>,
      "rentByUnit": <array of monthly rent per unit using HUD FMR for that unit's bedroom count — e.g. [1686, 1978] for 1BR+2BR>,
      "rentPerUnit": <number — average rent per unit (for reference only)>,
      "estimated": <number — TOTAL monthly gross rent = SUM of rentByUnit. For 1BR($1686)+2BR($1978) duplex = $3664. NEVER just multiply a single unit rent by unitCount without checking the unit mix.>,
    ...
    "expenses": {
      ...
      "propertyTaxes": {"annual": <number — use 1.0-1.5% of ARV for FL>, "monthly": <number>},
      "insurance": {"annual": <number — FL SFR budget $2,000-5,000/yr; more in flood/coastal zones>, "monthly": <number>},
...
We are active fix-and-flip operators in Tampa Bay. Our standards:
• SPEED: We close in 10-21 days cash, no mortgage contingency. This is a competitive advantage.
• TARGET: $150K-$500K purchase price, SFR or small multifamily (2-4 units), 1,200sf+ preferred.
...
OUTPUT: ONLY valid JSON, no markdown, no extra text..`;
  ...
  // ── ENFORCE MULTI-UNIT RENT VIA HUD FMR + UNIT MIX (server-side) ──────────────
  try {
    if (underwrite.rental && deal.propertyType) {
      const _units = getUnitCount(deal.propertyType);
      const _mr = underwrite.rental.marketRent || {};
      if (_units > 1) {
        // Calculate rent from unit mix × per-unit HUD FMR
        const _mix = calcRentFromUnitMix(deal.beds, deal.baths, _units, deal.county);
        _mr.unitCount  = _units;
        _mr.unitMix    = _mix.mix;    // e.g. [1, 2] for 1BR+2BR duplex
        _mr.rentByUnit = _mix.rents;  // e.g. [1686, 1978]

        // If Claude's estimate is clearly per-unit (single-unit rent), override it
        const _est = _mr.estimated || 0;
        const _hudTotal = _mix.total;
        // Claude wrong if it returned less than 75% of what HUD FMR says for all units
        if (!_est || _est < _hudTotal * 0.75) {
          _mr.estimated   = _hudTotal;
          _mr.hudFMR      = Math.round(_hudTotal / _units); // per-unit HUD FMR reference
          _mr.rentPerUnit = Math.round(_hudTotal / _units);
          _mr.source      = 'HUD FMR (unit mix: ' + _mix.mix.map(b => b + 'BR').join('+') + ')';
          _mr.confidence  = 'HIGH';
          console.log('[RENTAL] ' + _units + '-unit mix ' + JSON.stringify(_mix.mix) +
            ': rents ' + JSON.stringify(_mix.rents) + ' = $' + _hudTotal + ' total (was $' + _est + ')');
        }
        underwrite.rental.marketRent = _mr;
        ...
        // SFR — still sanity-check against HUD FMR
        const _hudFmr = getHudFmr(deal.county, deal.beds);
        if (!_mr.hudFMR) _mr.hudFMR = _hudFmr;
        // If Claude's estimate is way below HUD FMR (< 70%), flag it
        if (_mr.estimated && _mr.estimated < _hudFmr * 0.70) {
          console.log('[RENTAL] SFR est $' + _mr.estimated + ' below HUD FMR $' + _hudFmr + ' — bumping');
          _mr.estimated = _hudFmr;
          _mr.source = 'HUD FMR (AI estimate was too low)';
        }
      }
    }
  } catch(e) { console.log('[RENTAL] enforcement err:', e.message); }
...
app.get('/api/deals', auth, async (req, res) => {
  try {
    ...
    const targetDeals = deals.filter(d => isTargetCounty(d.county, d.city));
