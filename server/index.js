...
` +
  ...
  // If we got fewer than 3 sold comps, do a wider search
  if (sold.length < 3) {
    console.log("Web comps: only " + sold.length + " from narrow search — widening to zip/city level...");
    const r3 = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        tools: [{ type: 'web_search_20250305', name: 'web_search' }],
        messages: [{ role: 'user', content:
          `Search Zillow or Redfin for recently SOLD single family homes in ${city || ''} FL ${zip || ''} in the past 12 months. ` +
          `I need at least 3-5 sold comps. Price range $80K-$600K, 900-3500 sqft. ` +
          `Return ONLY JSON array: [{"address":"123 Oak","city":"${city||''}","zip":"${zip||''}","sqft":1500,"beds":3,"baths":2,"salePrice":245000,"saleDate":"2025-01","distanceMiles":0.8,"ppsf":163,"source":"zillow_sold"}]`
        }]
      })
    }).then(r => r.json()).catch(() => ({ content: [] }));
    parseComps(r3);
    console.log("Web comps after wide search: " + comps.length + " total");
  }

  const sold2 = comps.filter(c => !c.source?.includes('zestimate')).sort((a,b) => (a.distanceMiles||1) - (b.distanceMiles||1));
  const estimates2 = comps.filter(c => c.source?.includes('zestimate'));
  const result2 = [...sold2.slice(0, 8), ...estimates2.slice(0, 1)];
  console.log("Web comps: " + result2.length + " results for " + address + " (" + sold2.length + " sold + " + estimates2.length + " est)");
  return result2;
}
...
PUT THESE FIELDS FIRST — they are most important:
{
  ...
  "arv": {
    ...
    "asIsValue": <what the property is worth TODAY in as-is condition — no renovation, just cleaned/maintained enough to pass inspection. Use P50 of sold comps as-is. This is what a cash buyer would pay today. Always provide this.>,
    "urbanARV": <number>,
...
4. COMP HIERARCHY — use in order:
   ...
   C. Florida $/sqft benchmarks (only when A and B fail):
...
6. CONFIDENCE SIGNAL: arvConfidence = HIGH (5+ real comps, tight range) | MEDIUM (2-4 comps OR benchmarks used) | LOW (< 2 comps, wide range, must flag).
7. In arvNotes: (a) list comps used with address/price/sqft, (b) $/sqft you calculated, (c) how you got from comps to asIsValue and urbanARV, (d) confidence and why.
8. NEVER anchor to wholesaler ARV. Derive entirely independently. If wholesaler is >10% above your number, flag as INFLATED.
