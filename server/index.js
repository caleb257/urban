...
// Updated: 2026-06-13T05:56:13.772Z
...
async function fetchCountyGISComps(address, city, state, zip, county, beds, baths, sqft) {
  ...
  const deg = 0.0072; // ~0.5 miles in degrees lat/lng at FL latitudes
  ...
  const countyConfigs = {
    ...
    polk: {
      url: 'https://maps.polkpa.org/server/rest/services/Parcel/MapServer/0/query',
  ...
  try {
    const params = new URLSearchParams({
      ...
      orderByFields: 'SALE_DATE DESC,SALE_PRICE DESC',
    ...
    console.log(`🏛️ ${countyNorm} GIS: ${features.length} raw records within radius`);
