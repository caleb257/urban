...
async function fetchWebComps(address, city, zip, deal = {}) {
  ...
  // 3 parallel searches for maximum comp coverage
  const [r1, r2, r3] = await Promise.all([

    // Search 1: Redfin recently sold by zip — most reliable source
    fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages: [{ role: 'user', content:
          `Go to Redfin and find recently sold single family homes in zip code ${zip} FL sold in the past 12 months.
` +
          `Search URL to try: https://www.redfin.com/zipcode/${zip}/filter/include=sold-12mo,min-beds=${bedsLow},max-beds=${bedsHigh},min-sqft=${sqftLow},max-sqft=${sqftHigh}
` +
          `Also try searching: "${city || zip} FL recently sold homes ${beds||3} bedroom redfin"

` +
          `I need 5-8 SOLD homes (not for sale). Each must have the actual CLOSED SALE PRICE in dollars.
` +
          `Return ONLY a JSON array. Do NOT include active listings or asking prices:
` +
...
` +
          `salePrice must be the dollar amount (e.g. 248000 not "$248K"). Return [] if no actual closed sales found.`
        }]
      })
    }).then(r => r.json()).catch(() => ({ content: [] })),

    // Search 2: Zillow sold comps
    fetch('https://api.anthropic.com/v1/messages', {
      method: 'POST',
      signal: AbortSignal.timeout(25000),
      headers: { 'Content-Type': 'application/json', 'x-api-key': process.env.ANTHROPIC_API_KEY, 'anthropic-version': '2023-06-01', 'anthropic-beta': 'web-search-2025-03-05' },
      body: JSON.stringify({
        ...
        tools: [{ type: 'web_search_20250305', name: 'web_search' }],
        messages: [{ role: 'user', content:
          `Search Zillow for recently SOLD homes in ${city || zip} FL ${zip} — sold in 2024 or 2025.
` +
          `I need homes with ${bedsLow}-${bedsHigh} bedrooms, ${sqftLow}-${sqftHigh} sqft, single family.
` +
          `Try: "zillow.com ${zip} sold" OR "zillow ${city||''} FL recently sold ${beds||3} bedroom"

` +
          `Return ONLY actual closed sale transactions (not asking prices, not for-sale listings).
` +
          `JSON array only:
` +
...
` +
          `Return [] if no actual SOLD properties found.`
        }]
      })
    }).then(r => r.json()).catch(() => ({ content: [] })),

    // Search 3: Property appraiser or MLS sold comps
    fetch('https://api.anthropic.com/v1/messages', {
      method: 'POST',
      signal: AbortSignal.timeout(25000),
      headers: { 'Content-Type': 'application/json', 'x-api-key': process.env.ANTHROPIC_API_KEY, 'anthropic-version': '2023-06-01', 'anthropic-beta': 'web-search-2025-03-05' },
      body: JSON.stringify({
        ...
        tools: [{ type: 'web_search_20250305', name: 'web_search' }],
        messages: [{ role: 'user', content:
          `Find recent home sale prices near ${address}, ${city} FL ${zip}.
` +
          `Look for: ${city||zip} property appraiser recent sales, OR "${zip} FL home sales 2024 2025"
` +
          `I need single family homes, ${bedsLow}-${bedsHigh} bed, ${sqftLow}-${sqftHigh} sqft.
` +
          `Return sold prices as JSON array:
` +
...
` +
          `Return [] if no sale prices found.`
        }]
      })
    }).then(r => r.json()).catch(() => ({ content: [] }))
  ]);
  ...
  const parseComps = (result) => {
    ...
    try {
      ...
      for (const comp of arr) {
        // Robust price extraction — handle string prices like "$248K" or "248,000"
        let price = comp.salePrice;
        if (typeof price === 'string') {
          price = parseFloat(price.replace(/[$,K]/g, '')) * (price.toUpperCase().includes('K') ? 1000 : 1);
        }
        if (!price || price < 30000 || price > 10000000) continue;
        if (typeof price !== 'number' || isNaN(price)) continue;
        
        const key = ((comp.address || '') + (comp.zip || '')).toLowerCase().trim();
        if (seen.has(key) || !key) continue;
        seen.add(key);
        
        const compSqft = parseInt(comp.sqft) || null;
        const ppsf = compSqft && price ? Math.round(price / compSqft) : (comp.ppsf || null);
        comps.push({ ...comp, salePrice: Math.round(price), sold_price: Math.round(price), ppsf });
      }
    } catch (e) { /* silent */ }
  };
  ...
  // Normalize: ensure sold_price = salePrice for UI display
  comps.forEach(c => { if (!c.sold_price && c.salePrice) c.sold_price = c.salePrice; });

  // If fewer than 3 sold comps, do one more wider search
  const sold = comps.filter(c => !c.source?.includes('zestimate'));
  if (sold.length < 3) {
    console.log('Web comps: only ' + sold.length + ' from initial searches — trying wider area...');
    const r4 = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        tools: [{ type: 'web_search_20250305', name: 'web_search' }],
        messages: [{ role: 'user', content:
          `I urgently need real estate comparable sales data for ${city||''} FL zip ${zip}.
` +
          `Search for: home sales in ${city||zip} Florida 2024 2025 OR "${zip} property values sold"
` +
          `Any single family home sales in this area work. Return what you find as JSON:
` +
          `[{"address":"any address","city":"${city||''}","zip":"${zip||''}","sqft":1500,"beds":3,"baths":2,"salePrice":250000,"saleDate":"2024-06","source":"public_record"}]
` +
          `Return [] only if truly no data exists.`
        }]
      })
    }).then(r => r.json()).catch(() => ({ content: [] }));
    parseComps(r4);
  ...
  const soldFinal = comps.filter(c => !c.source?.includes('zestimate')).sort((a,b) => (a.distanceMiles||1) - (b.distanceMiles||1));
  const estimates = comps.filter(c => c.source?.includes('zestimate'));
  const result = [...soldFinal.slice(0, 10), ...estimates.slice(0, 1)];
  console.log("Web comps: " + result.length + " total for " + address + " (" + soldFinal.length + " sold)");
  return result;
}
...
PUT THESE FIELDS FIRST — they are most important:
{
  ...
  "arv": {
    ...
    "asIsValue": <REQUIRED: property value TODAY as-is with zero renovation. Cleaned out, pass-inspection ready. Use P50/median of sold comps. If no real comps, estimate as 80-85% of urbanARV. This is our downside/wholesale-out value. Never null.>,
    "urbanARV": <number>,
...
• SWEET SPOT: 3bd/2ba, 1,400-2,000sf, block/CBS construction, 1970-2000 vintage, light-to-medium rehab.
• AVOID: >$35K rehab without clear ARV support, tenant-occupied (eviction risk), flood zone (AE), mobile homes, weird lots, extreme rural.
...
app.get('/api/deals', auth, async (req, res) => {
  try {
    ...
    const targetDeals = deals.filter(d => {
      if (!d.county) return true; // if no county field, include (don't filter by city alone)
      return isTargetCounty(d.county);
    });
...
app.get('/api/stats', auth, (req, res) => {
  ...
  const allUw = Object.values(underwrites).filter(u => u.verdict && (!u.deal?.county || isTargetCounty(u.deal?.county)));
    ...
    if (dbCount > 0) {
      ...
      const targetFromDB = Object.fromEntries(
        Object.entries(fromDB).filter(([k, v]) => {
          const county = v.deal?.county || '';
          return !county || isTargetCounty(county);
        })
      );
...
async function serverAutoUnderwrite() {
  ...
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      // Filter by county field only (not city — Tampa in Hillsborough etc.)
      const _dealCounty = col('County') >= 0 ? (row[col('County')] || '') : '';
      if (_dealCounty && !isTargetCounty(_dealCounty)) {
        // Non-target county explicitly set — skip
        continue;
      }
      // If no county field, allow through (will be validated after deal is built)
      {
