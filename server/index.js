...
app.get('/', (req, res) => {
  ...
  res.setHeader('Expires', '0');
  res.setHeader('Surrogate-Control', 'no-store');
  res.type('html').send(EMBEDDED_HTML.replace('<\/body>','<scr'+'ipt src="/urban-patch.js"><\/scr'+'ipt><\/body>'));
});
...
async function regenerateVerdict(uw) {
  ...
  uw.financials = {
    ...
    askingPrice:        asking,
    rehabBudget:        repairs,
    hardMoney: asking > 0 ? { loanAmount: loanAmt, interestRate: 9.5, monthlyPayment: monthlyInterest, originationPoints: Math.round(loanAmt*0.02), totalInterest: Math.round(monthlyInterest*holdMonths+loanAmt*0.02) } : (uw.financials?.hardMoney||{}),
    holdingCosts: { total: holdCosts, insurance: 0, taxes: 0, utilities: Math.round(holdMonths*400) },
    sellingCosts: { total: sellCosts, agentCommission: Math.round(arv*0.06), closingCosts: Math.round(arv*0.02) },
    totalCost: asking>0 ? asking+repairs+costs : 0,
...
app.get('/api/stats', auth, (req, res) => {
  ...
  const scores = all.map(u => parseInt(u.score)||0).filter(s => s > 0);
